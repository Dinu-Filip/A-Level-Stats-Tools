...
class Normal:
    ...
    def cdf(mean_val: str, sd_val: str, lower_x_val: str, upper_x_val: str, dp: str) -> dict:
        ...
        if lower_x_val == "":
            ...
            method = r"""
                    We know that in general for continuous random variables:\n
                    $P(X \geq x) = 1 - P(X \leq x)$
                    So substituting into the equation we have
                    $P(X \geq x) = 1 - P(X \leq #x#) = #res#$
                    Since the integral in the normal CDF is not analytically tractable, it must be evaluated numerically.
                    """
        else:
    ...
    @staticmethod
    def normal_points(mean: str, sd: str) -> dict:
        mean = float(mean)
        sd = float(sd)
        min_x = mean - 5 * sd
        max_x = mean + 5 * sd
        points = {}
        for i in range(min_x, max_x, (max_x - min_x) / 100):
            points[str(i)] = norm.pdf(i, mean, sd)
        return points

...
class DistributionHT:
    ...
    def normal_cr(params: dict, type_test: str, type_tail=None):
        ...
        if type_test == "one_tailed":
            if type_tail == "lower":
                cv = norm.ppf(sig_level, loc=mean, scale=new_sd)
                return [("-\infty", cv)]
            else:
                cv = norm.ppf(1 - sig_level, loc=mean, scale=new_sd)
                return [(cv, "\infty")]
        else:
            cv_1 = norm.ppf(sig_level / 2, loc=mean, scale=new_sd)
            cv_2 = norm.ppf(1 - sig_level / 2, loc=mean, scale=new_sd)
    ...
    def binomial_p_value(params: dict, type_test, type_tail=None):
        ...
        print(type_test)
        print(type_tail)
        if type_test == "one_tailed" and type_tail == "upper":
    ...
    def calculate_cr(distribution: str, params: dict, type_test: str, type_tail: str):
        ...
        if distribution == "binomial":
            ...
            method_3 += r"X \approx N(#mean#, \frac{#sd#^2}{N}); note that we divide the population variance by the sample size"
...
Since $P(X \geq x) = 1 - P(X \leq x - 1)$ for discrete distributions, we have $P(X \leq x - 1) = """ + str(
            ...
            method_3 += r"Therefore the critical region is $[" + str(cr[0][0]) + ", " + str(cr[0][1]) + "]$"
        else:
...
To calculate $a$, we can directly use the inverse #distribution# function; this gives $a = """ + str(cr[0][1]) + "$."
    ...
    def get_p_value(distribution: str, params: dict, type_test: str, type_tail=None):
        step_3 = r"Calculate p-value assuming null hypothesis is true"
        p_val = None
        match distribution:
    ...
    def get_hypothesis(type_test: str, type_tail=None):
        ...
        if type_test == "one_tailed":
            if type_tail == "lower":
                ...
                step_2 += r"#population_param_name# > #population_param_value#$"
            method_2 = r"For a one_tailed test, there is a single critical region"
        else:
    ...
    def get_test_result_p_val(p_val, sig_level, type_test):
        step_4 = "Compare to the significance level"
        if type_test == "one_tailed":
    ...
    def binomial_actual_sig_level(type_test, type_tail, cr, params):
        ...
        if type_test == "one_tailed":
            ...
            actual_sig_level = round(
                binom.cdf(cr[0][1], num_trials, prob) + (1 - binom.cdf(cr[1][0], num_trials, prob)))
    ...
    def calculate_actual_sig_level(distribution, type_test, type_tail, cr, params):
        ...
        return actual_sig_level, step_6, method_6
    ...
    def distribution(distribution: str, type_test: str, params: dict):
        ...
        result = {"res": {"outcome": result,
                          "critical_region": cr,
                          "actual_sig_level": actual_sig_level},
...
if __name__ == "__main__":
    print(DistributionHT.distribution("normal", "two_tailed", {"test_stat": "8.95",
                                                                 "distribution": "normal",
                                                                 "population_param_name": r"\mu",
                                                                 "population_param_value": "9",
                                                               "mean": "9",
                                                                 "N": "30",
                                                                 "sd": "0.15",
                                                                 "sig_level": "0.05"}))

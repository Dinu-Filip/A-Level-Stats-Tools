...
\rho = \frac{#s_xy#}{\sqrt{#s_xx# \times #s_yy#}} = #res#"""
    ...
    def generate_results(x_vals: list, y_vals: list, dp: str):
        result = {}
        result["x_sum"] = BivariateAnalysis.sum(x_vals, dp)
        result["y_sum"] = BivariateAnalysis.sum(y_vals, dp)
        result["x_mean"] = BivariateAnalysis.mean(x_vals, dp)
        result["y_mean"] = BivariateAnalysis.mean(y_vals, dp)
        result["x_square_sum"] = BivariateAnalysis.square_sum(x_vals, dp)
        result["y_square_sum"] = BivariateAnalysis.square_sum(y_vals, dp)
        result["x_sd"] = BivariateAnalysis.standard_dev(x_vals, dp)
        result["y_sd"] = BivariateAnalysis.standard_dev(y_vals, dp)
        result["rho"] = BivariateAnalysis.pmcc(x_vals, y_vals, dp)
        result["regress_slope"] = BivariateAnalysis.regress_slope(x_vals, y_vals, dp)
        result["regress_intercept"] = BivariateAnalysis.regress_intercept(x_vals, y_vals, dp)
        return result
